...
export function appendClothesRail(
  input: { width: number; thickness: number },
  ...
  notes.push(`Лост за дрехи ${lengthMm} мм (${metres} м) · ${hardwareSettings.clothesRailEurPerM} €/м.`)
  hardware.push(
    pricedLine(
      { id: CLOTHES_RAIL.id, name: CLOTHES_RAIL.name, unitPriceEur: price },
      1,
      `${lengthMm} мм · ${hardwareSettings.clothesRailEurPerM} €/м`,
    ),
  )
}
...
export function appendShelves(
  ...
  notes.push(
    `${input.shelfCount} ${input.shelfCount === 1 ? 'рафт' : 'рафта'} с еднакви празнини по ${Math.round(gap)} мм.`,
  )
  notes.push(
...
export function appendDoorsAndDrawers(
  ...
  if (hasDoors && door) {
    ...
    notes.push(
      hasDrawers
        ? `${doorCount === 1 ? 'Една врата' : 'Две врати'}: рязане ${Math.round(door.width)} × ${Math.round(door.height)} мм (фуга 5 мм отгоре, 3 мм между челата, кант 2 мм от 4 страни).`
        : `${doorCount === 1 ? 'Една врата' : 'Две врати'}: рязане ${Math.round(door.width)} × ${Math.round(door.height)} мм (фуга 5 мм само отгоре, кант 2 мм от 4 страни).`,
    )
    notes.push(
